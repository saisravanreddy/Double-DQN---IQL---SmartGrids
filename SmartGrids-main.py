...
for i in range(num_of_agents - 1):
    agents.append(DoubleDQN_Agent_Price_Constant(names[i], state_size, max_battery, max_energy_generated, max_received,
                                                 min_non_adl, max_non_adl, grid_price, total_iterations - 30000, 0,
                                                 lamb[i]))
agents.append(
    DoubleDQN_Agent(names[2], state_size, max_battery, max_energy_generated, max_received, min_non_adl, max_non_adl,
                    grid_price, total_iterations - 30000, 0, lamb[2]))
...
for Iter in range(total_iterations):
    ...
    for i in range(num_of_agents):
        agents[i].remember(states_adl[i], states_price[i], adl_actions[i], pricing_actions[i], rewards[i],
                           temp_states_adl[i], temp_states_price[i])

    # Sample a mini batch and train
    if (Iter + 1) % 4 == 0 and (Iter + 1) < 1000000:
        loss = [(0, 0), (0, 0), (0, 0)]
        if Iter > 7000:
            for i in range(num_of_agents):
                loss[i] = agents[i].replay(minibatchsize)
                reward_summary = tf.Summary()
                reward_summary.value.add(tag='Reward', simple_value=float(mean(total_reward_for_display[i])))
                agents[i].reward_summary_writer.add_summary(reward_summary, training_step)
                pricing_summary = tf.Summary()
                pricing_summary.value.add(tag='Pricing-Loss', simple_value=float(loss[i][0]))
                agents[i].pricing_summary_writer.add_summary(pricing_summary, training_step)
                # agents[i].pricing_summary_writer.flush()
                adl_summary = tf.Summary()
                adl_summary.value.add(tag='ADL-Loss', simple_value=float(loss[i][1]))
                agents[i].adl_summary_writer.add_summary(adl_summary, training_step)
            # agents[i].adl_summary_writer.flush()
            # Update target network to prediction network
            if (training_step) % (targetupdatefrequency) == 0:
                print("target network updated")
                for i in range(num_of_agents):
                    agents[i].update_target_models()
            training_step += 1
