...
class DoubleDQN_Agent:
    ...
    def __init__(self, name, state_size, max_battery, max_energy_generated, max_received, min_non_adl, max_non_adl,
        ...
        self.action_size_pricing = (
                                               max_battery + max_energy_generated) * 6 + max_received + 1  # 6 for grid price to (grid price - 5) and 1 for the zeroth state.
        self.action_size_adl = 8
        self.total_iterations = total_iterations
        self.current_iteration = current_iteration
        self.memory = collections.deque(maxlen=replaybuffersize)
        self.gamma = 0.90  # discount rate
        self.epsilon = 0.8  # exploration rate
        self.epsilon_min = 0
        self.regularizer_loss = 50  # Loss for penalising the impossible actions
        self.regularizer_factor = 0.07
        self.prediction_pricing_model = self._build_model_pricing()
        self.target_pricing_model = self._build_model_pricing()
        self.prediction_adl_model = self._build_model_adl()
        self.target_adl_model = self._build_model_adl()
        # initialize target models(both pricing and adl models) = prediction models(pricing and adl models respectively)
        self.update_target_models()
        self.summary()
        self.reward_summary_writer = tf.summary.FileWriter(
            "./logs/" + datetime.now().strftime("%Y%m%d-%H%M%S") + '-itrs-' + str(total_iterations) + '-ms-' + str(
                minibatchsize) + '-tuf-' + str(targetupdatefrequency) + "/train_reward-" + str(self.name))
        self.pricing_summary_writer = tf.summary.FileWriter(
            "./logs/" + datetime.now().strftime("%Y%m%d-%H%M%S") + '-itrs-' + str(total_iterations) + '-ms-' + str(
                minibatchsize) + '-tuf-' + str(targetupdatefrequency) + "/train_pricing-" + str(self.name))
        self.adl_summary_writer = tf.summary.FileWriter(
            "./logs/" + datetime.now().strftime("%Y%m%d-%H%M%S") + '-itrs-' + str(total_iterations) + '-ms-' + str(
                minibatchsize) + '-tuf-' + str(targetupdatefrequency) + "/train_adl-" + str(self.name))
    ...
    def custom_loss(self, y_true, y_pred):
        loss = tf.keras.backend.mean(
            tf.keras.backend.sum(tf.keras.backend.square(y_true - y_pred), axis=1))  # need to put axis
        return loss
    ...
    def _build_model_pricing(self):
        ...
        model.add(tf.keras.layers.Dense(32, input_shape=(5,), activation=tf.keras.activations.relu,
    ...
    def _build_model_adl(self):
        ...
        # nd, d, adl_state, t, gp
        model = tf.keras.models.Sequential()
        model.add(tf.keras.layers.Dense(16, input_shape=(5,), activation=tf.keras.activations.relu,
                                        kernel_regularizer=tf.keras.regularizers.l2(2e-4)))
    ...
    def price_max_Q(self, state, li, ui):
        ...
        for i in range(lower_bound_index, upper_bound_index + 1):
            if i <= self.max_received:
                ...
                possible_actions.append(i)
        output = self.target_pricing_model.predict(state)
        max_ = -50000000
        for i in possible_actions:
            if max_ < output[0][i]:
                max_ = output[0][i]
    ...
    def adl_max_Q(self, state):
        ...
        index = 0
        max_ = - 50000000
        output = self.target_adl_model.predict(np.asarray([state]))
        for i in possible_actions:
            if max_ < output[0][i]:
                max_ = output[0][i]
                index = i
    ...
    def replay(self, batch_size):
        ...
        for state_adl, state_price, action_adl, action_price, reward, next_state_adl, next_state_price in minibatch:
            ...
            lower_bound_index_ns, upper_bound_index_ns = self.pricing_convert_constraint_values_to_allowed_action_indices(
                next_state_price)
            target_price = reward / 180.0 + self.gamma * self.price_max_Q(next_state_price, lower_bound_index_ns,
                                                                          upper_bound_index_ns)
...
class DoubleDQN_Agent_Price_Constant:

    def __init__(self, name, state_size, max_battery, max_energy_generated, max_received, min_non_adl, max_non_adl,
                 grid_price, total_iterations, current_iteration, lam):
        self.name = name
        self.state_size = state_size
        self.max_battery = max_battery      #maximum energy a battery can charge in a timeslot
        self.max_energy_generated = max_energy_generated
        self.max_received = max_received    #maximum energy that can be received by this microgrid(combining battery and load)
        self.grid_price = grid_price
        self.action_size_pricing = (
                                               max_battery + max_energy_generated) * 6 + max_received + 1  # 6 for grid price to (grid price - 5) and 1 for the zeroth state.
        self.action_size_adl = 8
    ...
    def pricing_action(self, state):
        ...
        possible_actions = []
        for i in range(lower_bound_index, upper_bound_index + 1):
            if i <= self.max_received:                 #i = self.max_received implies no energy being sold or received
                possible_actions.append(i)
            elif ((i - self.max_received) % 6) == 0:   # %6 is the price = gridprice - %6
                possible_actions.append(i)
        if (check < self.epsilon):
            return np.random.choice(possible_actions)
        else:
    ...
    def replay(self, batch_size):
        ...
        for state_adl, state_price, action_adl, action_price, reward, next_state_adl, next_state_price in minibatch:
            ...
            lower_bound_index_ns, upper_bound_index_ns = self.pricing_convert_constraint_values_to_allowed_action_indices(
                next_state_price)
            target_price = reward / 180.0 + self.gamma * self.price_max_Q(next_state_price, lower_bound_index_ns,
                                                                          upper_bound_index_ns)  # Normalized Rewards
            """here use the prediction network should try to reach the target value only for the taken action, the other actions Q-values targets should be same as the prediction network
            """
            target_array_price = self.prediction_pricing_model.predict(dl_state)
            target_array_price[0][action_price] = target_price
            ...
            dl_adl_state, dl_adl_next_state = np.array([state_adl]), np.array([next_state_adl])
            target_adl = reward / 180.0 + self.gamma * self.adl_max_Q(next_state_adl)
            """here use the prediction network should try to reach the target value only for the taken action, the other actions Q-values targets should be same as the prediction network,adl max Q and pricing max Q use the target network for max over a' Q(s',a')
            """
            target_array_adl = self.prediction_adl_model.predict(dl_adl_state)
            target_array_adl[0][action_adl] = target_adl
